...
class RequestService {
  ...
  final cloudinary =
      Cloudinary("225548188944398", "S0s1UsBPu3luxg5afZX_LyBNv-U", "dinqa9wqr");
  final String _BASE_URL = "https://leafappx.herokuapp.com/upload";
  final String _DbBASE_URL = 'https://leafapi.azurewebsites.net/api/Leafs/';
  final String _DbBASE_URLDetail = 'https://leafapi.azurewebsites.net/api/LeafDetails';
  RequestService() {
    formData = FormData();
  }
  Future<List<LeafModel>?> getData() async {
    try {
      ...
      if (_response.statusCode == 200) {
        ...
        return parsed
            .map<LeafModel>((json) => LeafModel.fromJson(json))
            .toList();
      }
  ...
  Future<bool?> createDetail(
      int leafId, double cct, String url, DateTime time) async {
  ...
  Future<bool?> deleteData(int id) async {
    ...
    final response =
        await Dio().delete("$_DbBASE_URL$id", data: formData, queryParameters: {
      "LeafId": id,
    });
